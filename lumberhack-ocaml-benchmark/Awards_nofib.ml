...
and competitors__d0 _lh_competitors_arg1_0 =
  ...
  (if (a_1 <= b_1) then
    (let rec t_1_5 = ((enumFromTo__d0 (a_1 + 1)) b_1) in
      (let rec h_1_5 = a_1 in
        (fun f_1_5 -> 
          (`LH_C((f_1_5 h_1_5), ((map__d5 f_1_5) t_1_5))))))
  else
    (fun f_1_6 -> 
      (`LH_N)))
    ...
    | `LH_C(h_7, t_7) -> 
      (let rec t_8 = ((map__d0 f_7) t_7) in
        (let rec h_8 = (f_7 h_7) in
          (fun f_8 -> 
            (if (f_8 h_8) then
              (`LH_C(h_8, ((filter__d0 f_8) t_8)))
            else
              ((filter__d0 f_8) t_8)))))
    | `LH_N -> 
      (fun f_9 -> 
        (`LH_N)))
and map__d2 f_2 ls_1 =
  (match ls_1 with
    | `LH_C(h_2, t_2) -> 
      (let rec t_3 = ((map__d2 f_2) t_2) in
        (let rec h_3 = (f_2 h_2) in
          (fun ys_0 -> 
            (`LH_C(h_3, ((mappend__d2 t_3) ys_0))))))
    | `LH_N -> 
      (fun ys_1 -> 
        ys_1))
...
Command_unix.run (Bench.make_command [
  Bench.Test.create ~name:"original_Awards_nofib" (fun () -> ignore (let open Module_original in ((testAwards_nofib__d0 500))));
  Bench.Test.create ~name:"lumberhack_Awards_nofib" (fun () -> ignore (let open Module_lumberhack in ((testAwards_nofib__d0 500))));
  Bench.Test.create ~name:"lumberhack_pop_out_Awards_nofib" (fun () -> ignore (let open Module_lumberhack_pop_out in ((testAwards_nofib__d0 500))));
])
