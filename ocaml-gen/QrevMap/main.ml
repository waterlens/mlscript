...
Command_unix.run (Bench.make_command [
  ...
  Bench.Test.create ~name:"lumberhack_inlined_QrevMap" (fun () -> ignore (let open Module_lumberhack_inlined_.Module_lumberhack_inlined_(struct end) in (run ())));
  Bench.Test.create ~name:"lumberhack_flo_out_QrevMap" (fun () -> ignore (let open Module_lumberhack_flo_out_.Module_lumberhack_flo_out_(struct end) in (run ())));
])
