...
rot p a b c = p (vec_add a b) c (vec_sub (0, 0) b)
-- // TODO: proper brackets
...
tup2 (a, b) (c, d) = (a, b, c, d)
tile_to_grid arg = grid 16 16 arg
p arg = tile_to_grid p_tile arg
q arg = tile_to_grid q_tile arg
r arg = tile_to_grid r_tile arg
s arg = tile_to_grid s_tile arg
quartet a b c d = above 1 1 (beside 1 1 a b) (beside 1 1 c d)
t arg = quartet p q r s arg
cycle' p1 = quartet p1 (rot (rot (rot p1))) (rot p1) (rot (rot p1))
u arg = cycle' (rot q) arg
side1 arg = quartet nil nil (rot t) t arg
side2 arg = quartet side1 side1 (rot t) t arg
corner1 arg = quartet nil nil nil u arg
corner2 arg = quartet corner1 side1 (rot side1) u arg
pseudocorner arg = quartet corner2 side2 (rot side2) (rot t) arg
pseudolimit arg = cycle' pseudocorner arg
...
corner arg = nonet corner2 side2 side2 (rot side2) u (rot t) (rot side2) (rot t) (rot q) arg
squarelimit arg = cycle' corner arg
-- sof: to make it easier to compare outputs, format the vector pairs on sep. lines
showFourTupleofInt (a,b,c,d) = "(" ++ (string_of_int a) ++ "," ++ (string_of_int b) ++ "," ++ (string_of_int c) ++ "," ++ (string_of_int d) ++ ")"
fmt []     = "[]"
fmt (x:xs) = ((\s -> "[|" ++ s) . (\s -> (showFourTupleofInt x) ++ s) . (showl xs)) ""
  where
    showl []     s = ']':s
    showl (x:xs) s = ((\s -> ",|" ++ s) . (\s -> (showFourTupleofInt x) ++ s) . (showl xs)) s
  -- SLPJ Nov 99.
...
testFish_nofib n =
  map (\i -> let n = min 0 i in length (fmt (pseudolimit (0, 0) (640+n, 0) (0,640+n)))) (primId [0..n])
